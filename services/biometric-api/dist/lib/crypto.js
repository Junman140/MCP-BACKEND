...
function getKey() {
    ...
    throw new Error("TEMPLATE_ENCRYPTION_KEY must be 64 hex chars (32 bytes) for AES-256-GCM");
}
/** Encrypt template bytes for DB storage (envelope: iv + tag + ciphertext). */
export function encryptTemplate(plain) {
    const key = getKey();
...
export function decryptTemplate(blob) {
    const key = getKey();
